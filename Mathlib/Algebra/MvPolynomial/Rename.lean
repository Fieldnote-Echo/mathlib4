...
@[deprecated rename (since := "2026-01-02")]
theorem rename_eq (f : σ → τ) (p : MvPolynomial σ R) : rename f p = mapDomain (.mapDomain f) p :=
  rfl

theorem rename_injective (f : σ → τ) (hf : Function.Injective f) :
    Function.Injective (rename f : MvPolynomial σ R → MvPolynomial τ R) :=
  mapDomain_injective (Finsupp.mapDomain_injective hf)
